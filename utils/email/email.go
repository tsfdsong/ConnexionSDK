...
func simpleNewMessage(subject string, body string) *Message {
...
func SendEmail(to []string, subject, body string, account, pass string) error {
	smtpHost := "smtp.gmail.com"
	smtpPort := 587
	smtpPass := pass
	smtpUser := account

	m := simpleNewMessage(subject, body)
	m.From = mail.Address{Name: "From", Address: account}
	m.To = to

	emailauth := smtp.PlainAuth("", smtpUser, smtpPass, smtpHost)

	err := smtp.SendMail(smtpHost+":"+strconv.Itoa(smtpPort), //convert port number from int to string
		emailauth,
		m.From.Address, m.Tolist(), m.Bytes(),
	)

	if err != nil {
		logger.Logrus.WithFields(logrus.Fields{"ErrMsg": err.Error()}).Error("send email failed")
		return err
	}
	return nil
}
