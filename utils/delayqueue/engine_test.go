...
func TestEngine_ConsumeMulti(t *testing.T) {
	...
	defer e.Shutdown()
	err = e.queues.Add("ns-engine", "q4")
	if err != nil {
		t.Fatalf("Failed to add queue: %s", err)
	}
	err = e.queues.Add("ns-engine", "q5")
	if err != nil {
		t.Fatalf("Failed to add queue: %s", err)
	}
	time.Sleep(100 * time.Millisecond)
