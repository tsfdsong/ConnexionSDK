...
func NewJob(namespace, queue string, body []byte, ttl, delay uint32, tries uint16) Job {
	id := GenUniqueJobIDWithDelay(delay)
	return &jobImpl{
		namespace: namespace,
		queue:     queue,
		id:        id,
...
func NewJobWithID(namespace, queue string, body []byte, ttl uint32, tries uint16, jobID string) Job {
	delay, _ := ExtractDelaySecondFromUniqueID(jobID)
	return &jobImpl{
		namespace: namespace,
		queue:     queue,
		id:        jobID,
...
func (j *jobImpl) ElapsedMS() int64 {
	...
	ms, _ := ElapsedMilliSecondFromUniqueID(j.id)
	j._elapsedMS = ms
	return ms
}
...
func (j *jobImpl) UnmarshalBinary(data []byte) error {
	...
	j.id = BinaryToUniqueID(binaryID)
	ttlOffset := idOffset + 16
	...
	delay, err := ExtractDelaySecondFromUniqueID(j.id)
	if err != nil {
		return err
	}
	j.delay = delay
