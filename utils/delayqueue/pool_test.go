...
func TestPool_Add(t *testing.T) {
	p := NewPool(R)
	job := NewJob("ns-pool", "q1", []byte("hello msg 1"), 10, 0, 1)
...
func TestPool_Add2(t *testing.T) {
	p := NewPool(R)
	job := NewJob("ns-pool", "q2", []byte("hello msg 2"), 1, 0, 1)
	p.Add(job)
...
func TestPool_Delete(t *testing.T) {
	p := NewPool(R)
	job := NewJob("ns-pool", "q3", []byte("hello msg 3"), 10, 0, 1)
	p.Add(job)
...
func TestPool_Get(t *testing.T) {
	p := NewPool(R)
	job := NewJob("ns-pool", "q4", []byte("hello msg 4"), 50, 0, 1)
	p.Add(job)
