...
const (
	QueueManagerSubscribeChannel = "queues_pubsub_channel"
	QueueManagerPrefix           = "queues"
)
	...
	for {
		...
		case <-m.stop:
			if err := m.pubsub.Unsubscribe(dummyCtx, QueueManagerSubscribeChannel); err != nil {
				logger.Logrus.WithError(err).Error("queue manager unsubscribe error")
			}
			m.pubsub.Close()
			logger.Logrus.Info("queue watcher would be exited while the stop signal was received")
			return
		case msg := <-ch:
			if err := m.update(); err != nil {
